...
section .data
    prompt1 db "Input a signed value (at most 128 bits) to convert or hit Enter to exit.", NEWLINE, \
    ...
    p2Len equ $-prompt2
    invalidInput db "Invalid input", NEWLINE, NEWLINE
    invalidLen equ $-invalidInput
    exitMsg db "Thank you for using our number conversion tool!", NEWLINE
    exitLen equ $-exitMsg
...
section .bss
    ...
    radix resb 1
    numBits resb 1
    printBuf resb 131                       ; for printing other radices
...
_start:
    ...
    mov eax, 3                              ; read first input (signed int)
    mov ebx, STDIN
    mov ecx, input1
    mov edx, 130
    int 0x80
    mov [i1Len], eax
    cmp byte [input1 + eax - 1], NEWLINE
    jz cont1
    call _flush_stdin                       ; clear stdin

cont1:
    cmp byte [i1Len], 0                     ; exit if end of input or newline
    jz exit
    cmp byte [input1], NEWLINE
    jz exit

    movzx esi, byte [i1Len]                 ; if last char is '\n', decrease length
    dec esi
...
val1:
    ...
    mov eax, 3                              ; read second input (num bits)
    mov ebx, STDIN
    mov ecx, input2
    mov edx, 3
    int 0x80
...
cont2:
    cmp byte [i2Len], 0                     ; exit if end of input or newline
    jz exit
    cmp byte [input2], NEWLINE
    jz exit
...
printd_pop_loop:                            ; attach digits to printBuf
    ...
    cmp ecx, 0
    jnz printd_pop_loop
    mov byte [printBuf + esi], NEWLINE
    inc esi

    mov eax, 4                              ; print signed decimal
    mov ebx, STDOUT
    mov ecx, printBuf
    mov edx, esi
    int 0x80
